...
def get_next_question_difficulty_and_skill(student_id, current_skill_id):
    ...
    2. If there is history:
       ...
       - If Incorrect:
         - If failed at level 2 or 3:
           ...
           - If correct count >= 3: decrease by 1 level (max(1, failed_level - 1)).
           - Stay on current skill.
    ...
    if last_correct:
        ...
        if streak_len >= 3:
            ...
            target_diff = random.choices([2, 3], weights=[10, 90])[0]
        else:
        ...
        if last_diff in [2, 3]:
            ...
            if correct_count < 3:
                ...
                # Many correct answers: decrease by 1 level
                return current_skill_id, max(1, last_diff - 1)
        else:
